...
start_date <- 20240915
end_date <- 20240922
path <- "C:/Users/legen/Desktop/Lab Project/닐슨/데이터/검증자료/"
...
remove_quick_member_transitions <- function(data_list) {
  cleaned_data_list <- list()
  ...
  for (i in seq_along(data_list)) {
    ...
    # 시간 컬럼을 POSIXct로 변환
    df <- df %>%
      mutate(time_parsed = hms(time))
    
    rows_to_delete <- c()
    
    for (j in 1:(nrow(df) - 1)) {
      current_act <- df$act[j]
      next_act <- df$act[j + 1]
      
      # 조건: 같은 멤버가 5초 이내에 나갔다 들어오는 경우
      if (grepl("member \\d+ has exited", current_act) && grepl("member \\d+ has entered", next_act)) {
        ...
        if (!is.na(current_member) && !is.na(next_member) && current_member == next_member) {
          time_diff <- as.numeric(difftime(df$time_parsed[j + 1], df$time_parsed[j], units = "secs"))
          
          if (time_diff <= 5) {
            rows_to_delete <- c(rows_to_delete, j, j + 1)
          }
        }
      }
    }
    ...
    if (length(rows_to_delete) > 0) {
      cleaned_df <- df[-rows_to_delete, ]
    } else {
      cleaned_df <- df
    }
    
    # time_parsed 컬럼 제거 후 저장
    cleaned_data_list[[names(data_list)[i]]] <- cleaned_df %>%
      select(-time_parsed)
  }
...
# df0915_1 ~ df0922_1 데이터 처리
for (day in 15:22) {
  var_name <- paste0("df09", day, "_1")
  if (exists(var_name)) {
    data_list <- get(var_name)  # 데이터 가져오기
    cleaned_data_list <- remove_quick_member_transitions(data_list)
    new_var_name <- paste0("df09", day, "_2")  # 새로운 이름으로 저장
    assign(new_var_name, cleaned_data_list)
  }
}

# df0915_1[['1402738']]
# df0915_2[['1402738']] 잘 삭제됨 
...
#### df0915_2 ~ df0922_2 각 리스트에 adjust_time 적용 ####
for (day in 15:22) {
  var_name <- paste0("df09", day, "_2")
  if (exists(var_name)) {
    data_list <- get(var_name)  # 데이터 가져오기
    ...
    # 새로운 이름으로 저장
    new_var_name <- paste0("df09", day, "_3")
...
for (month_num in 15:22) {
  ...
  df_list_name <- paste0("df09", month_num, "_3");  df_list_new_name <- paste0("df09", month_num, "_4")   
...
#### 걸러진 가구 확인 및 저장 ####
filtered_uncovered_viewing <- list()

for (day in 15:22) {
  var_name <- paste0("df09", day, "_4")  # 데이터 이름
  if (exists(var_name)) {
    data_list <- get(var_name)  # 데이터 가져오기
    
    # "UV"로 필터링된 가구 ID 추출
    uv_panels <- names(data_list)[sapply(data_list, function(df) all(df$id == "UV"))]
    
    # 결과 저장
    filtered_uncovered_viewing[[paste0("202409", day)]] <- list(
      panels = uv_panels,
      total = length(uv_panels)
    )
  }
}

# 결과를 확인
filtered_uncovered_viewing


# df0915_4 ~ df0922_4 에서 걸러진 가구 삭제 후 _5에 저장 
for (day in 15:22) {
  # 변수 이름 동적으로 생성
  var_name <- paste0("df09", day, "_4")  # 원본 리스트
  new_var_name <- paste0("df09", day, "_5")  # 결과 저장할 리스트
  
  if (exists(var_name) && exists("filtered_uncovered_viewing")) {
    # 원본 리스트 가져오기
    data_list <- get(var_name)
    
    # 해당 날짜의 UV 패널 ID 가져오기
    uv_panels <- filtered_uncovered_viewing[[paste0("202409", day)]]$panels
    
    # UV 패널 제거
    filtered_list <- data_list[!(names(data_list) %in% uv_panels)]
    
    # 결과 저장
    assign(new_var_name, filtered_list)
  }
}
...
# member entered 행의 chn에 앞의 행 chn 가져오기
# chn 값이 0 또는 65535이면 NA 값으로 바꾸기
...
# df0915_5 ~ df0922_5 처리 및 _5 로 저장
for (day in 15:22) {
...
# df0915_5 ~ df0922_5 데이터 처리 후 df0915_6 ~ df0922_6로 저장
for (day in 15:22) {
  var_name <- paste0("df09", day, "_5")
  if (exists(var_name)) {
    data_list <- get(var_name)  # 데이터 가져오기
    data_list <- add_ariana_code(data_list, mapping_data = mapping_data)  # 아리아나코드 추가
    new_var_name <- gsub("_5$", "_6", var_name)  # 새로운 변수명 생성
    assign(new_var_name, data_list)  # 결과 저장
  }
}
...
all_data <- list(df0915_6, df0916_6, df0917_6, df0918_6, df0919_6, df0920_6, df0921_6, df0922_6)
...
# 1. start & end 변수 생성 함수
# 윤진 part
define_start_end <- function(data_list) {
  for (i in seq_along(data_list)) {
    data_list[[i]] <- data_list[[i]] %>%
      mutate(
        start = time,
...
remove_no_member <- function(data_list) {
  for (i in seq_along(data_list)) {
    df <- data_list[[i]]
    member_list <- c()
    member_column <- character(nrow(df))
    ...
    for (j in seq_len(nrow(df))) {
      ...
      if (grepl("^member \\d+ has entered$", act)) {
        ...
        member_list <- union(member_list, member_id)
      }
      
      if (grepl("^member \\d+ has exited$", act)) {
        member_id <- sub("member (\\d+) has exited", "\\1", act)
        member_list <- setdiff(member_list, member_id)
      }
...
# df0915_6 ~ df0922_6 처리 및 df0915_7 ~ df0922_7 저장
for (day in 15:22) {
  var_name <- paste0("df09", day, "_6")
  if (exists(var_name)) {
    data_list <- get(var_name)
    
    # 1단계: start & end 생성
    data_list <- define_start_end(data_list)
    
    # 2단계: Playback 로그 제거
    data_list <- remove_playback_logs(data_list)
    
    # 3단계: 멤버가 없는 로그 제거
    data_list <- remove_no_member(data_list)
    
    # 4단계: NA chn 로그 제거
    data_list <- remove_na_chn_logs(data_list)
    
    # 결과 저장
    new_var_name <- paste0("df09", day, "_7")
    assign(new_var_name, data_list)
  }
}
...
# 처리할 리스트 이름 생성
input_list_names <- paste0("df09", 15:22, "_8")
output_list_names <- paste0("df09", 15:22, "_9")
...
rm(list = ls())
save(list = ls(), file = "/Users/kaaaaaag_/RStudioProjects/Nielsen Project/1.MET.RData")
